...
/*  Copy key array to key_buff2 */
    ...
    for( i=0; i<NUM_KEYS; i++) {
    //__builtin_prefetch(&key_array[i+64], 0, 1);
...
/*  In this section, the keys themselves are used as their 
    ...
    for( i=0; i<NUM_KEYS; i++ ) {
        key_buff1[key_buff2[i]]++;  /* Now they have individual key   */
                                    /* population                     */
       //__builtin_prefetch(&key_buff1[key_buff2[(i+64)]]);                           
       //if(i+16<NUM_KEYS)__asm__("PRFM PLDL1KEEP, [%0]" : : "r" (&key_buff1[key_buff2[(i+16)]]));                                
...
/*  To obtain ranks of each key, successively add the individual key
    ...
    for( i=0; i<MAX_KEY-1; i++ ) {  
    	//__builtin_prefetch(&key_buff1[i+64], 0, 1); 
