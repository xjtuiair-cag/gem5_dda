/*--------------------------------------------------------------------
  ...
  This benchmark is an OpenACC C version of the NPB CG code.
...
#pragma acc data create(colidx[0:NZ+1],rowstr[0:NA+1+1],a[0:NZ+1]) \
	create(x[0:NA+2+1],z[0:NA+2+1],p[0:NA+2+1]) \
	create(q[0:NA+2+1],r[0:NA+2+1],w[0:NA+2+1])
{
/*--------------------------------------------------------------------
c  
c-------------------------------------------------------------------*/
	...
	#pragma acc update device(colidx[0:NZ+1])
	#pragma acc update device(rowstr[0:NA+1+1])
	#pragma acc update device(a[0:NZ+1])
...
/* rolled version */ 
	...
	for (j = 1; j <= lastrow-firstrow+1; j++) {
		...
		//if(k+31<NZ)__asm__("PRFM PLDL1KEEP, [%0]" : : "r" (&p[colidx[k+32]]));
...
c---------------------------------------------------------------------*/
	...
	//__asm__("PRFM PLDL1KEEP, [%0]" : : "r" (&p[colidx[k+16]]) );
	//__asm__("PRFM PLDL1KEEP, [%0]" : : "r" (&p[colidx[k+32]]) );
	//__builtin_prefetch(&p[colidx[k+32]],0,1);
            d = d + a[k]*z[colidx[k]];
	}
	w[j] = d;
	r[j] = d;
